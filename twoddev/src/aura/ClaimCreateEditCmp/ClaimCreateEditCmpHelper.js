({
            ...
            case "accordion-c-1-save":
                {
                    ...
                    if (claimCustomRecordTypeName == 'Preventive Maintenance') {
                        ...
                        var schedule =component.get('v.scheduleSetValue');
                        if(schedule == '' || schedule == undefined || schedule == null){
                        	component.set('v.error.accordion1.fieldError.upcomingScheduler', $A.get("$Label.c.Upcoming_Scheduler_Entry_Not_Correct"));
                            temporaryValidationObject.upcomingScheduler = false;
                        }
                        
                        if (component.get('v.error.accordion1.fieldError.upcomingScheduler') !== '' && component.get('v.error.accordion1.fieldError.upcomingScheduler') != undefined) {
                        	temporaryValidationObject.upcomingScheduler = false;
                        }
                    }
                    
                     //Added code by chethan- to make "Hour Meter Reading" field Required..
                    if(claimCustomRecordTypeName == 'Serialized'){//modified by dharmil
                        if(currentClaim['Units_Run__c'] == '' || currentClaim['Units_Run__c'] == undefined || currentClaim['Units_Run__c'] == null){
                    ...
                    if (claimCustomRecordTypeName !== 'Claim Template') {
                        ...
                        /*if(claimCustomRecordTypeName == 'Preventive Maintenance' && new Date(failureDate) < new Date(serviceEligibleOnwards)){
                        ...
                        if (unitUsage === '' || unitUsage === undefined || unitUsage === null) {
                        	if(claimCustomRecordTypeName !== 'Non-Serialized'){//added by Dharmil
                            ...
                            if (claimCustomRecordTypeName == 'Serialized' || claimCustomRecordTypeName == 'Field Modification' || claimCustomRecordTypeName == 'Preventive Maintenance') {
                                var inventory = component.get('v.inventory');
    ...
    getInventoryUpcomingSchedule: function(component, memberList, defaultIndex) {
        ...
        component.set('v.upcomingScheduleList',memberList);
        
        

    },
    
    upadteInventoryRuleSchedule: function(component, claimId, ruleId){
		var getInventorySchedule = component.get('c.updateInventorySchedule');
        getInventorySchedule.setParams({
                        ClaimId: claimId,
                        ruleId: ruleId,
                    });
        
        getInventorySchedule.setCallback(this, function(response) {
                        var state = response.getState();
                        if (state === "SUCCESS" && component.isValid()) {
                            var returnedvalue = response.getReturnValue();
                            var response = [];
                    ...
                    $A.enqueueAction(getInventorySchedule);
        component.set('v.isInventoryScheduleEditable', false);
    },
    
    fetchUpcomingScheduleForClaim: function(component, claimId) {
        var getUpcomingSchedule = component.get('c.getUpcomingSchedule');
        getUpcomingSchedule.setParams({
            ClaimId: claimId,
        });
        
        getUpcomingSchedule.setCallback(this, function(response) {
                        var state = response.getState();
                        if (state === "SUCCESS" && component.isValid()) {
                            var returnedvalue = response.getReturnValue();
                            //var response = [];
                            //response = JSON.parse(returnedvalue);
                            var Name = returnedvalue.Name;
                            var Description = returnedvalue.Description__c;
                            var PreventativeMaintenanceDate = returnedvalue.Preventative_Maintenance_Date__c;
                            component.set('v.selectedSchedule',Name);
                            component.set('v.inventoryScheduleDescription',Description);
                            component.set('v.inventoryScheduleDate',PreventativeMaintenanceDate);
                            component.set('v.scheduleSetValue',Name);
                        }
                    });
                    $A.enqueueAction(getUpcomingSchedule);
    },
    
    finalSchedule : function(component, memberList,callbackFunction){
    	
    	var currentClaim = JSON.parse(JSON.stringify(component.get('v.currentClaim')));
    	var dateOfFailure = currentClaim['Date_of_Failure__c'];
    	var inventory = component.get('v.inventory');
    	var installDate = inventory['Install_Date__c'];
    	var claimUnitsRun = currentClaim['Units_Run__c'];
    	var responseValue={};
    	var getFinalSchedule = component.get('c.getFinalSchedule');
    	getFinalSchedule.setParams({
    		schedulerList: memberList,
    		failureDate: dateOfFailure,
    		invInstallDate: installDate,
    		claimUnitsRun: claimUnitsRun,
    	});
    	
    	 getFinalSchedule.setCallback(this, function(response) {
                        var state = response.getState();
                        if (state === "SUCCESS" && component.isValid()) {
                            var returnedvalue = response.getReturnValue();
                            component.set('v.finalScehdule',returnedvalue);
                            }
                            else if(state === "ERROR"){
                    ...
                    $A.enqueueAction(getFinalSchedule);
                            
    },
    
    fetchClaimTemplate: function(component, event, helper){
        var claimCustomRecordTypeName = component.get('v.claimCustomRecordTypeName');
        var currentClaim = JSON.parse(JSON.stringify(component.get('v.currentClaim')));
        ...
        var todayDateString = helper.getTodayDate(component, event);
                    ...
                    var selectedClaimTemplateFprPM = '';
                    ...
                    for(var i=0;i<inventoryScheduleList.length;i++){
                        if(selectedPreventiveMaintainanceRule === inventoryScheduleList[i]['Id']){
                            ...
                            selectedClaimTemplateFprPM = inventoryScheduleList[i]['Claim_Template__c'];
                        }
                    }
                    
                    
                    var selectedUpcomingScheduler = event.getSource().get('v.value');
                    var currentClaim = JSON.parse(JSON.stringify(component.get('v.currentClaim')));
                    //component.set('v.currentClaim.Campaign_Members__c', selectedUpcomingScheduler['Id']);
                    //component.set('v.campaignMember', selectedUpcomingScheduler);
                    
                    //get the claim template
                    var claimTemplateList=[] ;
                    if(selectedClaimTemplateFprPM != null && selectedClaimTemplateFprPM != ''){
                        
                        
                        var query = "Select Id,Name,CasualPart_Comments__c,Causal_Part_Number__c,Causal_Part_Number__r.Name,Causal_Part_Number__r.Id,Fault_Code_Comment__c,Fault_Code__c,Fault_Code__r.Name,Fault_Code__r.Id,Total_Category1_Cost__c,Total_Category2_Cost__c,Total_Meals_Cost__c,Total_Parking_Cost__c,TravelByDistance__c,TravelByHours__c,TravelByLocation__c,work_Performed_comments__c" +
                            " from Claim__c where Id = '" + selectedClaimTemplateFprPM+"\'";
                        
                        helper.readRaw(component, event, helper, query, function(claimTemplateResponse) {
                            claimTemplateList = claimTemplateResponse.sObjectList;
                            
                            //helper.getInventoryUpcomingSchedule(component, campaignMemberList, 0);
                            
                            if(claimTemplateList.length > 0){
                                
                                var claimTemplate = claimTemplateList[0];
                                component.set('v.claimTemp',claimTemplate);
    ...
    fetchGracePeriodCustomSettingRecord:function(component, event, helper, callbackFunction) {
        var gracePeriodValue = component.get('c.getGracePeriodCustomSetting');
        var responseValue={};
        gracePeriodValue.setCallback(this, function(response) {
            var state = response.getState();
            
            if (state === "SUCCESS" && component.isValid()) {
                var returnedvalue = response.getReturnValue();
                component.set('v.gracePeriod',returnedvalue);
            }else if(state === "ERROR"){
            		var exceptionObject=readAction.getError();
                    responseValue = {
                        "sObjectList": null,
        ...
        $A.enqueueAction(gracePeriodValue);
    },
    ...
    /***Negative Cost Validation method for Expense Details - 24 Jan 2017***/
    validateNegativeNumber: function(component, event){
        //alert("bmbgj");
		var inputTravHr = component.find("trav_hr");
        var valueTravHr = inputTravHr.get("v.value");
		
		var inputTravDist = component.find("trav_dist");
        var valueTravDist = inputTravDist.get("v.value");
		
		var inputFrieghtCharge = component.find("freight_charges");
        var valueFrieghtCharge = inputFrieghtCharge.get("v.value");
		
		var inputMiscelCharge = component.find("miscel_charges");
        var valueMiscelCharge = inputMiscelCharge.get("v.value");
        
		/** Travel By Hours **/
        if(valueTravHr < 0 ){
            document.getElementById("travelHrError").style.display="block";
            inputTravHr.set("v.value",0);
        }
        else{
            document.getElementById("travelHrError").style.display="none";
        }
		
		/** Travel By Distance **/
        if(valueTravDist < 0 ){
           document.getElementById("travelDistError").style.display="block";
           inputTravDist.set("v.value",0); 
        }
        else{
            document.getElementById("travelDistError").style.display="none";
        }
		
		/** Frieght Charges **/
        if(valueFrieghtCharge < 0 ){
            document.getElementById("frieghtError").style.display="block";
            inputFrieghtCharge.set("v.value",0); 
        }
        else{
            document.getElementById("frieghtError").style.display="none";
        }
		
		/** Miscellaneous Charges **/
        if(valueMiscelCharge < 0 ){
            document.getElementById("miscellError").style.display="block";
            inputMiscelCharge.set("v.value",0);
        }
        else{
            document.getElementById("miscellError").style.display="none";
        }
